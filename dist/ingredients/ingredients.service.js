...
let IngredientsService = class IngredientsService {
    ...
    async createInitialIngredients() {
        ...
        for (const ingredientItem of [...new Set(newIngredients)]) {
    ...
    async createMany(createIngredientsDto) {
        const newIngredients = createIngredientsDto.map((ingredientDto) => {
            const ingredient = new ingredient_entity_1.Ingredient();
            ingredient.name = ingredientDto.name;
            return ingredient;
        });
        const ingredientsRaw = (await this.ingredientRepository
            .createQueryBuilder()
            .insert()
            .into('ingredient')
            .values(newIngredients)
            .execute()).raw;
        const ingredientsId = ingredientsRaw.map((rawItem) => {
            return rawItem.id;
        });
